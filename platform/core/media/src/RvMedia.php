...
class RvMedia
{
    ...
    public function uploadFromUrl(
        ...
        // Auto-detect YouTube/Vimeo URLs
        ...
        // VR360: skip downloading entirely, create placeholder with stored URL
        if ($mediaType === 'vr360') {
            $extraOptions = ['vr360_url' => $originalUrl, 'media_type' => 'vr360'];
            return $this->createPlaceholderMediaFile($originalUrl, $folderId, $folderSlug, $extraOptions);
        }

        // For YouTube URLs, download thumbnail instead of the page
        if ($mediaType === 'video' && preg_match('/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/', $url, $matches)) {
            $url = 'https://img.youtube.com/vi/' . $matches[1] . '/hqdefault.jpg';
            $extraOptions = ['video_url' => $originalUrl, 'media_type' => 'video'];
        }
        // For Vimeo URLs, download thumbnail
        elseif ($mediaType === 'video' && preg_match('/vimeo\.com\/(?:video\/)?(\d+)/', $url, $matches)) {
            $url = 'https://vumbnail.com/' . $matches[1] . '.jpg';
            $extraOptions = ['video_url' => $originalUrl, 'media_type' => 'video'];
        }
        // For other video URLs (direct .mp4 etc), mark as video
        elseif ($mediaType === 'video') {
            $extraOptions = ['media_type' => 'video'];
        }
        ...
        // Sanitize filename: remove query strings and fragments, use temp dir
        $parsedUrl = parse_url($url);
        $urlPath = $parsedUrl['path'] ?? '';
        $basename = basename($urlPath) ?: ('download_' . time());
        $basename = preg_replace('/[^\w.\-]/', '_', $basename);

        $tempDir = sys_get_temp_dir();
        $path = $tempDir . DIRECTORY_SEPARATOR . Str::limit($basename, 50, '');
        ...
        // Store extra options (video_url, vr360_url, media_type) on the created file
        if (! $result['error'] && ! empty($extraOptions)) {
            $file = $result['data']->resource;
            $currentOptions = is_array($file->options) ? $file->options : [];
            $file->options = array_merge($currentOptions, $extraOptions);
            $file->save();
            $result['data'] = new FileResource($file->fresh());
        }

        return $result;
    }

    protected function createPlaceholderMediaFile(
        string $originalUrl,
        int|string $folderId,
        ?string $folderSlug,
        array $extraOptions
        ...
        // Build a descriptive name from the URL
        $urlParts = parse_url($originalUrl);
        $hostname = preg_replace('/[^\w\-]/', '_', $urlParts['host'] ?? 'media');
        $mediaTypeLabel = $extraOptions['media_type'] ?? 'media';
        $tmpName = $mediaTypeLabel . '_' . $hostname . '_' . time() . '.' . $ext;
        $tmpPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . $tmpName;

        // Copy the existing default placeholder image — avoids any GD dependency
        $defaultImagePath = public_path(ltrim(
            $this->getConfig('default_image', '/vendor/core/core/base/images/placeholder.png'),
            '/'
        ));

        if (File::exists($defaultImagePath)) {
            File::copy($defaultImagePath, $tmpPath);
        } else {
            // Minimal 1×1 transparent PNG (raw bytes, no GD required)
            file_put_contents($tmpPath, base64_decode(
                'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII='
            ));
        }

        $fileUpload = new UploadedFile($tmpPath, $tmpName, $mimeType, null, true);
        $result = $this->handleUpload($fileUpload, $folderId, $folderSlug);
        File::delete($tmpPath);
